...
public class BMICalculator
{
     ...
     * y: age - 5
     */
    private static readonly float[,] boyBMI = {
     ...
     * y: age - 5
     */
    private static readonly float[,] girlBMI = {
     ...
     */
    public static int getBMIResult(bool isGirl, int age, float patientBMI)
    {
        int ageIndex = age - 5;

        if (ageIndex < 0 || ageIndex > boyBMI.GetLength(1) - 1)
        {
            return 4;
        }
        else if (isGirl)
     ...
     */
    public static String getBMIResultString(bool isGirl, int age, float patientBMI)
    {
    ...
    {
        if (patientBMI < bmiChart[0, ageIndex])
        { //less than 5P
            return 0;
        }
        else if (patientBMI > bmiChart[2, ageIndex])
        { //greater or equal to 95P
            return 3;
        }
        else if (patientBMI > bmiChart[1, ageIndex])
        { //85P to less than 95P
            return 2;
        }
        else
        {
